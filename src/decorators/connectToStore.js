...
export default (storeNames, getStateFromStores) => (Component) => class ConnectToStore extends ReactComponent {
    ...
    componentDidMount = () => {
    	storeNames.map(name => stores[name]).forEach(store => store.addChangeListener(this.handleStoresChanged))
    }

    componentWillUnmount = () => {
    	storeNames.map(name => stores[name]).forEach(store => store.removeChangeListener(this.handleStoresChanged))
    }
